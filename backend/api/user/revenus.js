...
router.get("/revenus", async (req, res, next) => {
  const alias = "r";
...
router.get("/revenu/:id", async (req, res, next) => {
  const id = req.params.id;

  try {
    const revenu = await getOrSetCache(`revenu_${id}`, async () => {
      const [revenu_fetched] = await db
        .select("*")
        .from("Revenus")
        .where("id", id)
      if (!revenu_fetched) return notFound(next, "Revenu");
      const invoices = await db
        .select("*")
        .from("Invoices")
        .where("RevenuId", revenu_fetched.id);
      revenu_fetched.Invoices = invoices;
      const credits = await db
        .select("*")
        .from("Credits")
        .where("RevenuId", revenu_fetched.id);
      revenu_fetched.Credits = credits;
      const costs = await db
        .select("*")
        .from("Costs")
        .where("RevenuId", revenu_fetched.id);
      revenu_fetched.Costs = costs;
      const quotations = await db
        .select("*")
        .from("Quotations")
        .where("RevenuId", revenu_fetched.id);
      revenu_fetched.Quotations = quotations;
      const transactions = await db
        .select("*")
        .from("Transactions")
        .where("RevenuId", revenu_fetched.id);
      revenu_fetched.Transactions = transactions;
...
router.put("/revenu/:id", async (req, res, next) => {
